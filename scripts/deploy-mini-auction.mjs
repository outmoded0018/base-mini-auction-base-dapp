...
const source = await readFile(resolve(process.cwd(), "contracts/BaseMiniAuction.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: {
    "BaseMiniAuction.sol": { content: source },
  },
  settings: {
    outputSelection: {
      "*": {
...
const contract = output.contracts?.["BaseMiniAuction.sol"]?.BaseMiniAuction;
if (!contract) {
...
console.log(`Deploying BaseMiniAuction from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_MINI_AUCTION_CONTRACT_ADDRESS to this contract address in Vercel.");
